...
import org.team_trk.Objects.Product_maas;
...
public class OrderProcessing extends BaseAgent {
	...
	private HashMap<String, Product_maas> hmProducts; // = Available Products
	...
	protected void setup() {
		...
		this.register(sBakeryId + "-OrderProcessing", this.sBakeryId);
		findScheduler();
	...
	private class OfferRequestServerNew extends Behaviour {
		...
		public void action() {
			...
			case 0:
				...
				break;
			case 1:
				MessageTemplate schedulerReply = MessageTemplate.and(
				...
				if (schedulerMessage != null) {
					...
					if (schedulerMessage.getPerformative() == ACLMessage.CONFIRM) {
						...
						for (String product_name : order.getProducts().keySet()) {
							double priceAllProductsOfType = hmProducts.get(product_name).getSalesPrice()
									* order.getProducts().get(product_name);
		...
		private void distributeScheduledOrder() {
			...
			MessageTemplate acceptedProposalMT = MessageTemplate.or(
	...
	private boolean readArgs(Object[] oArgs) {
		if (oArgs != null && oArgs.length > 0) {
			...
			while (product_iterator.hasNext()) {
				...
				Product_maas product = new Product_maas(jsoProduct.toString());
