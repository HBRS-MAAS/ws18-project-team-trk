...
//        System.out.println("SchedulerAgent is ready");
	...
	private boolean checkDoughPrepStage(Order order, HashMap<String, List<Pair<String, Integer>>> tempPrepTables,
			HashMap<String, List<Pair<String, Integer>>> tempKneadingMachines) {
...
//        HashMap<String, List<Pair<String, Integer>>> tempKneadingMachines = new HashMap<>();
		...
		for (String pName : orderProducts) {
			ProductMas p = hmProducts.get(pName);
...
//                            System.out.println(getName() + ": no kneadingMachineFound!");
					...
					if (!alreadyIn && !dayPlan.isEmpty()) {
						int latestTime = tempKneadingMachines.get(chosenKM)
								.get(tempKneadingMachines.get(chosenKM).size() - 1).getValue();
...
//                        System.out.println(getName() + ": endingTime: " + endingTime);
	...
	private String checkItemPrepTables(int itemPrepTime, int startingTime,
			HashMap<String, List<Pair<String, Integer>>> tempPrepTables, int deadline) {
		...
		while (ptIterator.hasNext()) {
			...
			if (dailyPlan.get(dailyPlan.size() - 1).getValue() < dailyPlanEarliest.get(dailyPlanEarliest.size() - 1)
					.getValue()) {
				earliestPrepTable = actualPt;
			}
	...
	private String checkKneadingMachineTimes(int kneadingTime, String productName,
			HashMap<String, List<Pair<String, Integer>>> tempKneadingMachines, int deadline) {
		...
		if (tempKneadingMachine.isEmpty()
				|| (tempKneadingMachine.get(tempKneadingMachine.size() - 1).getValue() + kneadingTime) < deadline) {
			return earliestKneadingMachine;
		}
		return null;
	}

	private String checkIfTypeAlreadyKneaded(String productName,
			HashMap<String, List<Pair<String, Integer>>> tempKneadingMachines) {
...
//                        HashMap<String, List<Pair<String, Integer>>> tempPrepTables = new HashMap<>();
//                        HashMap<String, List<Pair<String, Integer>>> tempKneadingMachines = new HashMap<>();
					boolean doughPrepPossible = checkDoughPrepStage(new Order(sContent), tempPrepTables,
							tempKneadingMachines);
...
//                            System.out.println(myAgent.getName() + ": schedule reply sent!");
				...
				break;
			case 1:
				MessageTemplate accepted_proposalMT = MessageTemplate.and(
						MessageTemplate.MatchPerformative(ACLMessage.PROPAGATE),
						MessageTemplate.MatchSender(order_processing));
				MessageTemplate rejectProposalMT = MessageTemplate.and(
...
//                        System.out.println(myAgent.getName() + ": Scheduler Agent Propagated all scheduled Orders");
	...
	private class ScheduledOrderRequestServer extends CyclicBehaviour {
		...
		@Override
		public void action() {
			MessageTemplate allOrderRequestMT = MessageTemplate.and(
					MessageTemplate.MatchPerformative(ACLMessage.REQUEST),
	...
	public static HashMap<String, Order> sortOrders(HashMap<String, Order> hm) {
		...
		Collections.sort(orders, new Comparator<Map.Entry<String, Order>>() {
			...
			public int compare(Map.Entry<String, Order> o1, Map.Entry<String, Order> o2) {
				...
				if (o1.getValue().getDeliveryDay() == o2.getValue().getDeliveryDay()
						&& o1.getValue().getOrderDay() < o2.getValue().getOrderDay()) {
					return -1;
				}
				if (o1.getValue().getDeliveryDay() == o2.getValue().getDeliveryDay()
						&& o1.getValue().getOrderDay() > o2.getValue().getOrderDay()) {
...
//            lLocation = new Location(jsoLocation.getDouble("y"), jsoLocation.getDouble("x"));
			...
			timeStepSize = (24 * timeStep.getInt("day") + timeStep.getInt("hour")) * 60 + timeStep.getInt("minute");
			return true;
		} else {
			return false;
		}
